...
function keyPressed(){
	...
	if(!timerEnd && fightDisplay){
		...
		if(key === 'q' && player.allowCrouch && !player.healthChanged && (millis() > playerAttackTimer + 1500)){
			player.highAnimation = true;
			player.highAttack();
			playerAttackTimer = millis();
		} else if (key === 'e' && player.allowCrouch && !player.healthChanged && (millis() > playerAttackTimer + 1500)){
		...
		if(key === 'u' && enemy.allowCrouch && !enemy.healthChanged && (millis() > enemyAttackTimer + 1500)){
			enemy.highAnimation = true;
			enemy.highAttack();
			enemyAttackTimer = millis();
		} else if (key === 'o' && enemy.allowCrouch && !enemy.healthChanged && (millis() > enemyAttackTimer + 1500)){
